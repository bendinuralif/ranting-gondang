...
function TambahSejarahSingkat() {
  const [no, setNo] = useState("");
  const [nama, setNama] = useState("");
  const [tahun, setTahun] = useState(""); // Merubah state jabatan menjadi tahun
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const docRef = await addDoc(collection(db, "Sejarah"), {
        no,
        nama,
        tahun // Merubah jabatan menjadi tahun
      });
      console.log("Document written with ID: ", docRef.id);
      // Reset form fields after successful submission
      setNo("");
      setNama("");
      setTahun(""); // Merubah jabatan menjadi tahun
      ...
      <div className="flex justify-center items-center p-5 ">
        <form className="max-w-3xl mx-auto bg-gray-200 shadow-md rounded-lg p-8" onSubmit={handleSubmit}>
          ...
          <div className="grid gap-6 mb-6 md:grid-cols-1">
            <div>
              <label
                htmlFor="tahun" // Merubah jabatan menjadi tahun
                className="block mb-2 text-sm font-medium text-gray-900 dark:text-white"
              >
                Tahun
              </label>
              <input
                type="text"
                id="tahun" // Merubah jabatan menjadi tahun
                value={tahun} // Merubah jabatan menjadi tahun
                onChange={(e) => setTahun(e.target.value)} // Merubah jabatan menjadi tahun
                className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                placeholder="Masukan Tahun" // Merubah jabatan menjadi tahun
                required
              />
            </div>
          </div>
...
export default TambahSejarahSingkat;
