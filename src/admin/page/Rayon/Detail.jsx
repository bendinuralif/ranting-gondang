...
function DetailRayon() {
  ...
  const fetchData = async () => {
    ...
    try {
      const res = await retrieveData("Rayon", db);
      setData(res);
    } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const db = getFirestore(app);
      const { nama, rayon } = selectedItem; // Mengganti 'Rayon' menjadi 'rayon'
      const itemId = selectedItem.id;
      const itemRef = doc(db, "Rayon", itemId);
      await updateDoc(itemRef, {
        nama: nama, // Mengganti 'Rayon' menjadi 'rayon'
        rayon: rayon, // Mengganti 'Rayon' menjadi 'rayon'
      });
            ...
            <tbody>
              {paginatedData.map((item, index) => (
                <tr key={index} className="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                  ...
                  <td className="px-2 py-2">{item.rayon}</td> {/* Mengganti 'Rayon' menjadi 'rayon' */}
