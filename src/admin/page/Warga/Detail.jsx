...
function DetailWarga() {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      try {
        const mainCollectionRef = collection(db, "Warga");
  ...
  const fetchData = async () => {
    const db = getFirestore(app);
    try {
      const res = await retrieveData("Warga", db);
  ...
  const confirmDelete = async () => {
    try {
      const db = getFirestore(app);
      await deleteDoc(doc(db, "Warga", selectedItemToDelete.id)); // Hapus item yang telah dikonfirmasi
