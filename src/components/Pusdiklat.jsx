...
function Pusdiklat() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const res = await retrieveData('Pusdiklat');
        setData(res);
            ...
            <table className="w-full text-sm text-left text-gray-700 dark:text-gray-400">
              <thead className="text-xs uppercase bg-gray-300 dark:bg-gray-700 dark:text-gray-400">
                <tr>
                  ...
                  <th scope="col" className="px-4 py-3">Nama Pembina</th>
                  <th scope="col" className="px-4 py-3">Pusdiklat</th>
                </tr>
              </thead>
              <tbody>
                {data.map((item, index) => (
                  <tr key={index} className={`bg-${index % 2 === 0 ? 'gray-100' : 'white'} border-b dark:bg-gray-800 dark:border-gray-700`}>
                    ...
                    <td className="px-4 py-3">{item.pusdiklat}</td>
                  </tr>
                ))}
              </tbody>
            </table>
