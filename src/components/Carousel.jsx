        ...
        {images.map((image, index) => (
          <div key={index} className={`absolute inset-0 transition-opacity duration-700 ease-in-out ${index === activeIndex ? 'opacity-100' : 'opacity-0'}`}>
            ...
            <div className="absolute inset-0 flex items-center justify-center flex-col h-full text-center text-white">
              <span className="text-[1.3rem] font-semibold">PERSAUDARAAN SETIA HATI TERATE</span>
              <span className="text-[1.3rem] font-semibold">RANTING GONDANG</span>
              <span className="text-sm font-semibold mt-2">“Dengan Pencak Silat Kita Berbudaya Dan Berprestasi”</span>
            </div>
          </div>
        ))}
      </div>
